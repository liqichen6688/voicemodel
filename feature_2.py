...
ALPHA_LIST = [glob.glob(alpha_path + "/*.csv")[_][len(alpha_path):-4] for _ in range(len(glob.glob(alpha_path + "/*.csv")))]
...
#        r = np.append(r, volfeature, axis = 1).T
        ...
        for pathset in [store_path + '/' + 'x_feature_train'  ,store_path + '/' + 'x_feature_test'  , store_path + '/' + 'y_pretrain' ]:
...
#        print(prelabel)
        ...
        print(label)
        for pathset in [store_path + '/' + 'y_train' + '/', store_path + '/' + 'y_test' + '/']:
            if not(os.path.exists(pathset)):
                os.makedirs(pathset)
...
#     return nor_feature
def normalization(feature, window=NORM_WINDOW):
    '''
    feature.shape = (time, NUM_FEATURE*NUM_PERIOD)
    '''
