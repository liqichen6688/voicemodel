...
#        for z in symbol_list:
        if True:
            test_path = './data_backup/alpha/' + x  + '.csv'
...
#    return booksheet
def select_distinct(li1, bits1, bits2):
    temp = []
    for i in li1:
        if i[bits1 : bits2] not in temp:
            temp.append(i[bits1 : bits2])
    return temp
...
#    datalist = glob.glob('./data_backup/1minbar/*.*')
#    datalist = select_distinct(datalist, 29, 39)
#    datalist.sort()
    y_list = [ _ for _ in data_loader.collist]
    y_list.sort()
    for y in y_list:
