...
def han():
    ...
    input1 = Input(shape=(2, 14), dtype='float32')

    # Attention Layer
    dense_layer = Dense(14, activation='tanh')(input1)
    ...
    input2 = Input(shape=(5, 2, 14), dtype='float32')
...
# bidirectional gru
    l_gru = Bidirectional(GRU(7, return_sequences=True))(pre_gru)
...
# MLP to perform classification
    dense1 = Dense(20, activation='tanh')(post_gru)
...
def training(x_name,y_name,model):
    x_train = np.load('data_backup/feed_data/x_train/'+x_name)
...
def testing(x_test_folder, y_test_foloder,model):
    ...
    for duo in duo_test_list:
        x_test = np.load('data_backup/feed_data/x_test/'+duo[0])
...
if __name__ == "__main__":
    ...
    x_train_folder='data_backup/feed_data/x_train'
    y_train_folder='data_backup/feed_data/y_train'
    x_test_folder = 'data_backup/feed_data/x_test'
